...
# BEFORE ADDED EXECUTION SUMMARY TABLE IN COVER 
...
# AFTER ADDED EXECUTION SUMMARY TABLE IN COVER
...
def cover_page(request):
    ...
    # Total checklists per user per sprint
    grouped_data = (
        checklists
        .values("task__sprint_no", "created_by__username")
        .annotate(total_checklists=Count("sl_no"))
    ...
    # Status breakdown per user per sprint
    status_data = (
        checklists
        .values("task__sprint_no", "created_by__username")
        .annotate(
            not_executed=Count("sl_no", filter=Q(status="NOT_EXECUTED")),
            passed=Count("sl_no", filter=Q(status="PASSED")),
            failed=Count("sl_no", filter=Q(status="FAILED"))
        )
        .order_by("task__sprint_no", "created_by__username")
    )
    ...
    for item in status_data:
        sprint_no = item["task__sprint_no"]
        username = item["created_by__username"]
        not_exec = item["not_executed"]
        passed = item["passed"]
        failed = item["failed"]
        total = not_exec + passed + failed
        if sprint_no not in sprint_status_summary:
            sprint_status_summary[sprint_no] = []
        sprint_status_summary[sprint_no].append({
            "user": username,
    ...
    context = {
        ...
        "selected_sprint": selected_sprint,
        "sprint_summary": sprint_summary,
        "sprint_status_summary": sprint_status_summary
    }
    return render(request, "checklist/cover_page.html", context)
